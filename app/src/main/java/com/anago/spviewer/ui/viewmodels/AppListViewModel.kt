...
class AppListViewModel(private val app: Application) : AndroidViewModel(app) {
    private val mAppList: MutableLiveData<List<App>> = MutableLiveData<List<App>>(emptyList())

    fun getAppList(): LiveData<List<App>> {
        return mAppList
    }
    ...
    */
    fun loadAppList(onLoaded: (() -> Unit)? = null) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val appList = pm.getCInstalledApplications(0)
